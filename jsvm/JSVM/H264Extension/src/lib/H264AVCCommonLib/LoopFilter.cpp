                     ...
                     const MbStatus*          apcMbStatus )
{
  ...
  const CommonElt* pcSlice = dynamic_cast<const CommonElt*>(pcMainH264->getEltCurrent()); // should be the current slice.
  const CommonH264Section* pcSection = dynamic_cast<const CommonH264Section*>(pcMainH264->getSectionCurrent()); // should be a section with all slices. Important because LoopFilter is done after ALL slices are decoded.
  const CommonMb* pcEltMb;
  ...
  for( LFPass eLFPass = FIRST_PASS; eLFPass < TWO_PASSES; eLFPass = LFPass( eLFPass + 1 ) )
  {
    for( UInt uiMbAddress = 0; uiMbAddress < rcSH.getMbInPic(); uiMbAddress++ )
    {
	  ...
	  if(eLFPass == SECOND_PASS && pcSection)
	  {
		    // Find in the current slice
		    pcEltMb = pcSlice->findMb(uiMbAddress, pcMainH264->getCurrentLayerId());
			if (!pcEltMb)
			{
				// Find in previous slices (held in the current section)
				pcEltMb = pcSection->findMb(uiMbAddress, pcMainH264->getCurrentLayerId());
			}
			if (pcEltMb)
			{
				cYuvMbBuffer.loadBuffer( pcFrameBuffer );
				pcEltMb->setData(CommonEltMbDataType_Final, 
