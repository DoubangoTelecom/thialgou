...
//  const CommonSyntax* pcSyntax;
  ...
  const CommonControl* pcControlFor0 = pcEltCurrent->addControl("for" , "( i = 0; i < ( ( chroma_format_idc != 3 ) ? 8 : 12 ); i++ )", 1);
  pcMainH264->setEltCurrent(pcControlFor0);
  for( Int i4x4 = 0; i4x4 < 6; i4x4++ )
  {
    RNOK( m_acScalingMatrix4x4.get(i4x4).read( pcReadIf, g_aucFrameScan , i4x4 ) );
  }
  pcMainH264->setEltCurrent(pcEltCurrent);
  ROTRS( ! bRead8x8, Err::m_nOK );
  pcMainH264->setEltCurrent(pcControlFor0);

  RNOK  ( m_acScalingMatrix8x8.get(0   ).read( pcReadIf, g_aucFrameScan64, 6 ) );
  RNOK  ( m_acScalingMatrix8x8.get(1   ).read( pcReadIf, g_aucFrameScan64, 7 ) );
  pcMainH264->setEltCurrent(pcEltCurrent);
