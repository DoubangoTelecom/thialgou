...
RefPicListReOrdering::read( HeaderSymbolReadIf& rcReadIf, UInt uiNumRefIdx )
{
  ...
  pcSyntax = pcEltCurrent->addSyntax("ref_pic_list_modification_flag_l0/1", "u(1)");
  RNOK  ( rcReadIf.getFlag( m_bRefPicListReorderingFlag,  "RPLR: ref_pic_list_reordering_flag" ) );
	pcSyntax->setValue(m_bRefPicListReorderingFlag);
	const CommonControl* pcControlIf0 = pcEltCurrent->addControl("if" , "ref_pic_list_modification_flag_l0/1", m_bRefPicListReorderingFlag);
	
  ROFRS ( m_bRefPicListReorderingFlag, Err::m_nOK );
 
  const CommonFunction* pcFuncDo0 = pcControlIf0->addFunction("do while(modification_of_pic_nums_idc != 3)");
  pcMainH264->setEltCurrent(pcFuncDo0);
  Bool  bEnd = false;
  char buffer[33];
  for( UInt uiIndex = 0; !bEnd; uiIndex++ )
  {
	  itoa(uiIndex, buffer, 10);
	  const CommonFunction* pcFuncStep = pcFuncDo0->addFunction("step("+std::string(buffer)+")");
  pcMainH264->setEltCurrent(pcFuncStep);
...
DecRefPicMarking::read( HeaderSymbolReadIf& rcReadIf )
{
  ...
  if( m_bDecRefBasePicMarking )
  {
  pcSyntax = pcEltCurrent->addSyntax("adaptive_ref_pic_marking_mode_flag", "u(1)");
    RNOK( rcReadIf.getFlag( m_bAdaptiveRefPicMarkingModeFlag,  "DPRM:  adaptive_ref_base_pic_marking_mode_flag" ) );
	pcSyntax->setValue(m_bAdaptiveRefPicMarkingModeFlag);
  }
  else
  {
	   pcSyntax = pcEltCurrent->addSyntax("adaptive_ref_pic_marking_mode_flag", "u(1)");
  ...
  const CommonControl* pcControlIf0 = pcEltCurrent->addControl("if" , "adaptive_ref_pic_marking_mode_flag", m_bAdaptiveRefPicMarkingModeFlag);
  ROFRS ( m_bAdaptiveRefPicMarkingModeFlag, Err::m_nOK );
...
PredWeight::read( HeaderSymbolReadIf& rcReadIf, UInt uiLumaLog2WeightDenom, UInt uiChromaLog2WeightDenom )
{
  ...
  pcSyntax = pcEltCurrent->addSyntax("luma_weight_flag", "u(1)");
  RNOK  ( rcReadIf.getFlag( m_bLumaWeightFlag,   "PWT: luma_weight_flag" ) );
  pcSyntax->setValue(m_bLumaWeightFlag);
  
  const CommonControl* pcControlIf0 = pcEltCurrent->addControl("if" , "luma_weight_l1_flag", m_bLumaWeightFlag);
  if( m_bLumaWeightFlag )
  {
	  pcSyntax = pcControlIf0->addSyntax("luma_weight_l0/1[ i ]", "se(v)");
    RNOK( rcReadIf.getSvlc( m_iLumaWeight,       "PWT: luma_weight" ) );
	pcSyntax->setValue(m_iLumaWeight);
	 pcSyntax = pcControlIf0->addSyntax("luma_offset_l0/1[ i ]", "se(v)");
    RNOK( rcReadIf.getSvlc( m_iLumaOffset,       "PWT: luma_offset" ) );
	pcSyntax->setValue(m_iLumaOffset);
  ...
  const CommonControl* pcControlIf1 = pcEltCurrent->addControl("if" , "ChromaArrayType != 0", 1);
  pcSyntax = pcControlIf1->addSyntax("chroma_weight_l0/1_flag", "u(1)");
  RNOK  ( rcReadIf.getFlag( m_bChromaWeightFlag, "PWT: chroma_weight_flag" ) );
  pcSyntax->setValue(m_bChromaWeightFlag);
  const CommonControl* pcControlIf2 = pcControlIf1->addControl("if" , "chroma_weight_l0/1_flag", m_bChromaWeightFlag);
  if( m_bChromaWeightFlag )
  {
	  const CommonControl* pcControlFor0 = pcControlIf2->addControl("for" , "j = 0; j < 2; j++ ", 1);

	  pcSyntax = pcControlFor0->addSyntax("chroma_weight_l0/1[ i ][ j/*0*/ ]", "se(v)");
    RNOK( rcReadIf.getSvlc( m_iChromaCbWeight,   "PWT: chroma_weight (Cb)" ) );
	pcSyntax->setValue(m_iChromaCbWeight);
	pcSyntax = pcControlFor0->addSyntax("chroma_offset_l0/1[ i ][ j/*0*/ ]", "se(v)");
    RNOK( rcReadIf.getSvlc( m_iChromaCbOffset,   "PWT: chroma_offset (Cb)" ) );
	pcSyntax->setValue(m_iChromaCbOffset);
	pcSyntax = pcControlFor0->addSyntax("chroma_weight_l0/1[ i ][ j/*1*/ ]", "se(v)");
    RNOK( rcReadIf.getSvlc( m_iChromaCrWeight,   "PWT: chroma_weight (Cr)" ) );
	pcSyntax->setValue(m_iChromaCrWeight);
	pcSyntax = pcControlFor0->addSyntax("chroma_offset_l0/1[ i ][ j/*1*/ ]", "se(v)");
    RNOK( rcReadIf.getSvlc( m_iChromaCrOffset,   "PWT: chroma_offset (Cr)" ) );
	pcSyntax->setValue(m_iChromaCrOffset);
...
PredWeightTable::read( HeaderSymbolReadIf& rcReadIf, UInt uiNumRefIdxActiveMinus1, UInt uiLumaLog2WeightDenom, UInt uiChromaLog2WeightDenom )
{
  ...
  ASSERT(pcMainH264 != NULL);
  const CommonElt* pcEltCurrent = pcMainH264->getEltCurrent();
  const CommonControl* pcControlFor = pcEltCurrent->addControl("for" , "i = 0; i <= num_ref_idx_l0/1_active_minus1; i++", 1);

  pcMainH264->setEltCurrent(pcControlFor);
  for( UInt uiIndex = 0; uiIndex <= uiNumRefIdxActiveMinus1; uiIndex++ )
  {
...
DBFilterParameter::read( HeaderSymbolReadIf& rcReadIf, Bool bSVCNalUnit )
{
  ...
  const CommonControl* pcControlIf0 = pcEltCurrent->addControl("if" , "!InterLayerParameters", ! m_bInterLayerParameters);
  const CommonControl* pcControlElseIf0 = pcEltCurrent->addControl("else" , "", m_bInterLayerParameters);
  if( ! m_bInterLayerParameters )
  {
	  pcSyntax = pcControlIf0->addSyntax("disable_deblocking_filter_idc", "ue(v)");
    RNOK  ( rcReadIf.getUvlc( m_uiDisableDeblockingFilterIdc, "SH: disable_deblocking_filter_idc" ) );
	pcSyntax->setValue(m_uiDisableDeblockingFilterIdc);
	const CommonControl* pcControlIf1 = pcEltCurrent->addControl("if" , "disable_deblocking_filter_idc != 1", m_uiDisableDeblockingFilterIdc != 1);
    if( m_uiDisableDeblockingFilterIdc != 1 )
    {
		pcSyntax = pcControlIf1->addSyntax("slice_alpha_c0_offset_div2", "se(v)");
      RNOK( rcReadIf.getSvlc( m_iSliceAlphaC0OffsetDiv2,      "SH: slice_alpha_c0_offset_div2" ) );
	  pcSyntax->setValue(m_iSliceAlphaC0OffsetDiv2);
	  pcSyntax = pcControlIf1->addSyntax("slice_beta_offset_div2", "se(v)");
      RNOK( rcReadIf.getSvlc( m_iSliceBetaOffsetDiv2,         "SH: slice_beta_offset_div2" ) );
	  pcSyntax->setValue(m_iSliceBetaOffsetDiv2);
    }
  }
  else
  {
	  pcSyntax = pcControlElseIf0->addSyntax("disable_inter_layer_deblocking_filter_idc", "ue(v)");
    RNOK  ( rcReadIf.getUvlc( m_uiDisableDeblockingFilterIdc, "SH: disable_inter_layer_deblocking_filter_idc" ) );
	pcSyntax->setValue(m_uiDisableDeblockingFilterIdc);

	const CommonControl* pcControlIf2 = pcEltCurrent->addControl("if" , "disable_inter_layer_deblocking_filter_idc != 1", m_uiDisableDeblockingFilterIdc != 1);
    if( m_uiDisableDeblockingFilterIdc != 1 )
    {
		pcSyntax = pcControlIf2->addSyntax("inter_layer_slice_alpha_c0_offset_div2", "se(v)");
      RNOK( rcReadIf.getSvlc( m_iSliceAlphaC0OffsetDiv2,      "SH: inter_layer_slice_alpha_c0_offset_div2" ) );
	  pcSyntax->setValue(m_iSliceAlphaC0OffsetDiv2);
	  pcSyntax = pcControlIf2->addSyntax("inter_layer_slice_beta_offset_div2", "se(v)");
...
SliceHeaderSyntax::read( ParameterSetMng& rcParameterSetMng, HeaderSymbolReadIf& rcReadIf, Bool bInclusiveNalUnitHeader )
{
  ...
  ASSERT(pcMainH264 != NULL);
  const CommonElt* pcEltCurrent = pcMainH264->getEltCurrent();
  ASSERT(pcEltCurrent != NULL && pcEltCurrent->getType() == CommonEltType_Nalu);
  CommonHeader* _pThialgouSlice = new CommonHeader(CommonHeaderType_Slice, "slice_header"), *pThialgouSlice = _pThialgouSlice;
  pcEltCurrent->addElt((CommonElt**)(&_pThialgouSlice));
   const CommonSyntax* pcSyntax;
  ...
  pcSyntax = pThialgouSlice->addSyntax("first_mb_in_slice", "ue(v)");
  RNOK(         rcReadIf.getUvlc ( m_uiFirstMbInSlice,                             "SH: first_mb_in_slice" ) );
  pcSyntax->setValue(m_uiFirstMbInSlice);

  UInt uiSliceType;
  pcSyntax = pThialgouSlice->addSyntax("slice_type", "ue(v)");
  RNOK(         rcReadIf.getUvlc ( uiSliceType,                                    "SH: slice_type" ) );
  pcSyntax->setValue(uiSliceType);
  m_eSliceType = SliceType( uiSliceType );
  
  pcSyntax = pThialgouSlice->addSyntax("pic_parameter_set_id", "ue(v)");
  ...
  pThialgouSlice->addControl("if" , "separate_colour_plane_flag == 1", 0);

  pcSyntax = pThialgouSlice->addSyntax("frame_num", "u(v)");
  RNOK(         rcReadIf.getCode ( m_uiFrameNum,
                                   getSPS().getLog2MaxFrameNum(),                  "SH: frame_num" ) );
  pcSyntax->setValue(m_uiFrameNum);

  const CommonControl* pcControlIf0 = pThialgouSlice->addControl("if" , "!frame_mbs_only_flag", ! getSPS().getFrameMbsOnlyFlag());
  if( ! getSPS().getFrameMbsOnlyFlag() )
  {
	  pcSyntax = pcControlIf0->addSyntax("field_pic_flag", "u(1)");
    RNOK(       rcReadIf.getFlag ( m_bFieldPicFlag,                                "SH: field_pic_flag" ) );
	pcSyntax->setValue(m_bFieldPicFlag);

	const CommonControl* pcControlIf1 = pcControlIf0->addControl("if" , "field_pic_flag", m_bFieldPicFlag);
    if( m_bFieldPicFlag )
    {
		pcSyntax = pcControlIf1->addSyntax("bottom_field_flag", "u(1)");
      RNOK(     rcReadIf.getFlag ( m_bBottomFieldFlag,                             "SH: bottom_field_flag" ) );
	  pcSyntax->setValue(m_bBottomFieldFlag);
    }
  }

  const CommonControl* pcControlIf2 = pThialgouSlice->addControl("if" , "IdrPicFlag", getIdrFlag());
  
  if( getIdrFlag() )
  {
	  pcSyntax = pcControlIf2->addSyntax("idr_pic_id", "ue(v)");
    RNOK(       rcReadIf.getUvlc ( m_uiIdrPicId,                                   "SH: idr_pic_id" ) );
	pcSyntax->setValue(m_uiIdrPicId);
  }

  const CommonControl* pcControlIf3 = pThialgouSlice->addControl("if" , "pic_order_cnt_type == 0", getSPS().getPicOrderCntType() == 0);
  if( getSPS().getPicOrderCntType() == 0 )
  {
	   pcSyntax = pcControlIf3->addSyntax("pic_order_cnt_lsb", "u(v)");
    RNOK(       rcReadIf.getCode ( m_uiPicOrderCntLsb,
                                   getSPS().getLog2MaxPicOrderCntLsb(),            "SH: pic_order_cnt_lsb" ) );
	pcSyntax->setValue(m_uiPicOrderCntLsb);

	const CommonControl* pcControlIf4 = pcControlIf3->addControl("if" , "bottom_field_pic_order_in_frame_present_flag && !field_pic_flag", getPPS().getPicOrderPresentFlag() && ! m_bFieldPicFlag);
    if( getPPS().getPicOrderPresentFlag() && ! m_bFieldPicFlag )
    {
		pcSyntax = pcControlIf4->addSyntax("delta_pic_order_cnt_bottom", "se(v)");
      RNOK(     rcReadIf.getSvlc ( m_iDeltaPicOrderCntBottom,                      "SH: delta_pic_order_cnt_bottom" ) );
	  pcSyntax->setValue(m_iDeltaPicOrderCntBottom);
    }
  }

  const CommonControl* pcControlIf5 = pThialgouSlice->addControl("if" , "pic_order_cnt_type == 1 && !delta_pic_order_always_zero_flag", getSPS().getPicOrderCntType() == 1 && !getSPS().getDeltaPicOrderAlwaysZeroFlag());
  if( getSPS().getPicOrderCntType() == 1 && !getSPS().getDeltaPicOrderAlwaysZeroFlag() )
  {
	  pcSyntax = pcControlIf5->addSyntax("delta_pic_order_cnt[0]", "se(v)");
    RNOK(       rcReadIf.getSvlc ( m_iDeltaPicOrderCnt0,                           "SH: delta_pic_order_cnt[0]" ) );
	pcSyntax->setValue(m_iDeltaPicOrderCnt0);

	const CommonControl* pcControlIf6 = pThialgouSlice->addControl("if" , "bottom_field_pic_order_in_frame_present_flag && !field_pic_flag", getPPS().getPicOrderPresentFlag() && ! m_bFieldPicFlag);
    if( getPPS().getPicOrderPresentFlag() && ! m_bFieldPicFlag )
    {
		pcSyntax = pcControlIf6->addSyntax("delta_pic_order_cnt[1]", "se(v)");
      RNOK(     rcReadIf.getSvlc ( m_iDeltaPicOrderCnt1,                           "SH: delta_pic_order_cnt[1]" ) );
	  pcSyntax->setValue(m_iDeltaPicOrderCnt1);
    }
  }

  const CommonControl* pcControlIf7 = pThialgouSlice->addControl("if" , "redundant_pic_cnt_present_flag", getPPS().getRedundantPicCntPresentFlag());
  if( getPPS().getRedundantPicCntPresentFlag() )
  {
	  pcSyntax = pcControlIf7->addSyntax("redundant_pic_cnt", "ue(v)");
    RNOK(       rcReadIf.getUvlc ( m_uiRedundantPicCnt,                            "SH: redundant_pic_cnt" ) );
	pcSyntax->setValue(m_uiRedundantPicCnt);
  }
  if( ! getQualityId() )
  {
	  const CommonControl* pcControlIf8 = pThialgouSlice->addControl("if" , "slice_type == B", isBSlice());
    if( isBSlice() )
    {
		pcSyntax = pcControlIf8->addSyntax("direct_spatial_mv_pred_flag", "u(1)");
      RNOK(     rcReadIf.getFlag ( m_bDirectSpatialMvPredFlag,                     "SH: direct_spatial_mv_pred_flag" ) );
	  pcSyntax->setValue(m_bDirectSpatialMvPredFlag);
    }

	const CommonControl* pcControlIf9 = pThialgouSlice->addControl("if" , "slice_type == P || slice_type == SP || slice_type == B", isInterSlice());
    if( isInterSlice() )
    {
		pcSyntax = pcControlIf9->addSyntax("num_ref_idx_active_override_flag", "u(1)");
      RNOK(     rcReadIf.getFlag ( m_bNumRefIdxActiveOverrideFlag,                 "SH: num_ref_idx_active_override_flag" ) );
	  pcSyntax->setValue(m_bNumRefIdxActiveOverrideFlag);

		const CommonControl* pcControlIf10 = pcControlIf9->addControl("if" , "num_ref_idx_active_override_flag", m_bNumRefIdxActiveOverrideFlag);
      if( m_bNumRefIdxActiveOverrideFlag )
      {
		  pcSyntax = pcControlIf10->addSyntax("num_ref_idx_l0_active_minus1", "ue(v)");
        RNOK(   rcReadIf.getUvlc ( m_uiNumRefIdxL0ActiveMinus1,                    "SH: num_ref_idx_l0_active_minus1" ) );
		pcSyntax->setValue(m_uiNumRefIdxL0ActiveMinus1);
		const CommonControl* pcControlIf11 = pcControlIf10->addControl("if" , "slice_type == B", isBSlice());
        if( isBSlice() )
        {
			pcSyntax = pcControlIf11->addSyntax("num_ref_idx_l1_active_minus1", "ue(v)");
          RNOK( rcReadIf.getUvlc ( m_uiNumRefIdxL1ActiveMinus1,                    "SH: num_ref_idx_l1_active_minus1" ) );
		  pcSyntax->setValue(m_uiNumRefIdxL1ActiveMinus1);
        }
      }
    }	

	const CommonControl* pcControlIf12 = pThialgouSlice->addControl("if", "nal_unit_type == 20", getPPS().getNalUnitType() == NAL_UNIT_CODED_SLICE_SCALABLE); // MVC (Annex H) always False
	const CommonControl* pcControlElseIf12 = pThialgouSlice->addControl("else", "", getPPS().getNalUnitType() != NAL_UNIT_CODED_SLICE_SCALABLE);
	const CommonFunction* pcFuncRefPicListModification = pcControlElseIf12->addFunction("ref_pic_list_modification");
	const CommonControl* pcControlIf13 = pcFuncRefPicListModification->addControl("if", "slice_type % 5 != 2 && slice_type % 5 != 4", isInterSlice() );
	const CommonControl* pcControlIf14 = pcFuncRefPicListModification->addControl("if", "slice_type % 5 == 1", isBSlice() );

	
    if( isInterSlice() )
    {
		pcMainH264->setEltCurrent(pcControlIf13);
      RNOK(     m_cRefPicListReorderingL0.read( rcReadIf ) );
    }
    if( isBSlice() )
    {
		pcMainH264->setEltCurrent(pcControlIf14);
      RNOK(     m_cRefPicListReorderingL1.read( rcReadIf ) );
    }
	pcMainH264->setEltCurrent(pcEltCurrent);

	const CommonControl* pcControlIf15 = pThialgouSlice->addControl("if", "(weighted_pred_flag && (slice_type == P || slice_type == SP)) || (weighted_bipred_idc == 1 && slice_type == B)", ( getPPS().getWeightedPredFlag() && isPorSPSlice() ) || ( getPPS().getWeightedBiPredIdc() == 1 && isBSlice() ) );
    if( ( getPPS().getWeightedPredFlag() && isPorSPSlice() ) || ( getPPS().getWeightedBiPredIdc() == 1 && isBSlice() ) )
    {
		const CommonControl* pcControlIf16 = pcFuncRefPicListModification->addControl("if", "!no_inter_layer_pred_flag", ! getNoInterLayerPredFlag() );
      if( ! getNoInterLayerPredFlag() )
      {
		  pcSyntax = pcControlIf16->addSyntax("base_pred_weight_table_flag", "u(1)");
        RNOK(   rcReadIf.getFlag ( m_bBasePredWeightTableFlag,                     "SH: base_pred_weight_table_flag" ) );
		pcSyntax->setValue(m_bBasePredWeightTableFlag);
      }

	  const CommonControl* pcControlIf17 = pcFuncRefPicListModification->addControl("if", "no_inter_layer_pred_flag || !base_pred_weight_table_flag", getNoInterLayerPredFlag() || ! m_bBasePredWeightTableFlag  );	  
      if( getNoInterLayerPredFlag() || ! m_bBasePredWeightTableFlag )
      {
		  const CommonFunction* pcFuncPredWeightTable = pcControlIf17->addFunction("pred_weight_table");

		  pcSyntax = pcFuncPredWeightTable->addSyntax("luma_log2_weight_denom", "ue(v)");
        RNOK(   rcReadIf.getUvlc ( m_uiLumaLog2WeightDenom,                        "PWT: luma_log_weight_denom" ) );
		pcSyntax->setValue(m_uiLumaLog2WeightDenom);

		const CommonControl* pcControlIf18 = pcFuncPredWeightTable->addControl("if", "ChromaArrayType != 0", 1);
		pcSyntax = pcControlIf18->addSyntax("chroma_log2_weight_denom", "ue(v)");
        RNOK(   rcReadIf.getUvlc ( m_uiChromaLog2WeightDenom,                      "PWT: chroma_log_weight_denom" ) );
		pcSyntax->setValue(m_uiChromaLog2WeightDenom);

		pcMainH264->setEltCurrent(pcFuncPredWeightTable);
        RNOK(   m_cPredWeightTableL0.read( rcReadIf, m_uiNumRefIdxL0ActiveMinus1, m_uiLumaLog2WeightDenom, m_uiChromaLog2WeightDenom ) );

		const CommonControl* pcControlIf19 = pcFuncPredWeightTable->addControl("if", "slice_type % 5 == 1", isBSlice());
        if( isBSlice() )
        {
			pcMainH264->setEltCurrent(pcControlIf19);
	...
	const CommonControl* pcControlIf20 = pThialgouSlice->addControl("if", "nal_ref_idc != 0", getNalRefIdc());
    if( getNalRefIdc() )
    {
		const CommonFunction* pcFuncDecRefPicMarking = pcControlIf20->addFunction("dec_ref_pic_marking");
		const CommonControl* pcControlIf21 = pcFuncDecRefPicMarking->addControl("if", "IdrPicFlag", getIdrFlag());
		const CommonControl* pcControlElseIf21 = pcFuncDecRefPicMarking->addControl("else", "", !getIdrFlag());
      if( getIdrFlag() )
      {
		  pcSyntax = pcControlIf21->addSyntax("no_output_of_prior_pics_flag", "u(1)");
        RNOK(   rcReadIf.getFlag ( m_bNoOutputOfPriorPicsFlag,                     "DRPM: no_output_of_prior_pics_flag" ) );
		pcSyntax->setValue(m_bNoOutputOfPriorPicsFlag);
		pcSyntax = pcControlIf21->addSyntax("long_term_reference_flag", "u(1)");
        RNOK(   rcReadIf.getFlag ( m_bLongTermReferenceFlag,                       "DRPM: long_term_reference_flag" ) );
		pcSyntax->setValue(m_bLongTermReferenceFlag);
      }
      else
      {
		  pcMainH264->setEltCurrent(pcControlElseIf21);
        RNOK(   m_cDecRefPicMarking.read( rcReadIf ) );
		pcMainH264->setEltCurrent(pcEltCurrent);
      }

	  const CommonControl* pcControlIf22 = pcFuncDecRefPicMarking->addControl("if", "!slice_header_restriction_flag", ! getSPS().getAVCHeaderRewriteFlag() && ! isH264AVCCompatible());
      if( ! getSPS().getAVCHeaderRewriteFlag() && ! isH264AVCCompatible() )
      {
        Bool bStoreRefBasePicFlag;
		 pcSyntax = pcControlIf22->addSyntax("no_output_of_prior_pics_flag", "u(1)");
        RNOK(   rcReadIf.getFlag ( bStoreRefBasePicFlag,                           "SH: store_ref_base_pic_flag" ) );
		pcSyntax->setValue(bStoreRefBasePicFlag);
        setStoreRefBasePicFlag( bStoreRefBasePicFlag );

		const CommonControl* pcControlIf23 = pcFuncDecRefPicMarking->addControl("if", "(use_ref_base_pic_flag || store_ref_base_pic_flag) && !idr_flag ", ( getUseRefBasePicFlag() || getStoreRefBasePicFlag() ) && !getIdrFlag());
        if( ( getUseRefBasePicFlag() || getStoreRefBasePicFlag() ) && !getIdrFlag() )
        {
			const CommonFunction* pcFuncDecRefBasePicMarking = pcControlIf23->addFunction("dec_ref_base_pic_marking");
			 pcMainH264->setEltCurrent(pcFuncDecRefBasePicMarking);
  ...
  const CommonControl* pcControlIf24 = pThialgouSlice->addControl("if", "entropy_coding_mode_flag && slice_type != I && slice_type != SI", getPPS().getEntropyCodingModeFlag() && !isIntraSlice() );
  if( getPPS().getEntropyCodingModeFlag() && !isIntraSlice() )
  {
	  pcSyntax = pcControlIf24->addSyntax("cabac_init_idc", "ue(v)");
    RNOK(       rcReadIf.getUvlc ( m_uiCabacInitIdc,                               "SH: cabac_init_idc" ) );
	pcSyntax->setValue(m_uiCabacInitIdc);
  }

  pcSyntax = pThialgouSlice->addSyntax("slice_qp_delta", "se(v)");
  RNOK(         rcReadIf.getSvlc ( m_iSliceQpDelta,                                "SH: slice_qp_delta" ) );
  pcSyntax->setValue(m_iSliceQpDelta);

   const CommonControl* pcControlIf25 = pThialgouSlice->addControl("if", "slice_type == SP | | slice_type == SI", isSPSlice() || isSISlice());
  if( isSPSlice() || isSISlice() )
  {
	  const CommonControl* pcControlIf26 = pcControlIf25->addControl("if", "slice_type == SP", isSPSlice());
    if( isSPSlice() )
    {
		pcSyntax = pcControlIf26->addSyntax("sp_for_switch_flag", "u(1)");
      RNOK(     rcReadIf.getFlag ( m_bSPForSwitchFlag,                             "SH: sp_for_switch_flag" ) );
	  pcSyntax->setValue(m_bSPForSwitchFlag);
    }
	pcSyntax = pcControlIf25->addSyntax("slice_qs_delta", "se(v)");
    RNOK(       rcReadIf.getSvlc ( m_iSliceQsDelta,                                "SH: slice_qs_delta" ) );
	pcSyntax->setValue(m_iSliceQsDelta);
  }

  const CommonControl* pcControlIf27 = pThialgouSlice->addControl("if", "deblocking_filter_control_present_flag", getPPS().getDeblockingFilterParametersPresentFlag());
  if( getPPS().getDeblockingFilterParametersPresentFlag() )
  {
	  pcMainH264->setEltCurrent(pcControlIf27);
    RNOK(       m_cDeblockingFilterParameter.read( rcReadIf, getNalUnitType() == NAL_UNIT_CODED_SLICE_SCALABLE ) );
	pcMainH264->setEltCurrent(pcEltCurrent);
  }

  const CommonControl* pcControlIf28 = pThialgouSlice->addControl("if", "num_slice_groups_minus1 > 0 && slice_group_map_type >= 3 && slice_group_map_type <= 5", getPPS().getNumSliceGroupsMinus1() > 0 &&
      getPPS().getSliceGroupMapType() >= 3 && getPPS().getSliceGroupMapType() <= 5);
  if( getPPS().getNumSliceGroupsMinus1() > 0 &&
      getPPS().getSliceGroupMapType() >= 3 && getPPS().getSliceGroupMapType() <= 5 )
  {
	  pcSyntax = pcControlIf28->addSyntax("slice_group_change_cycle", "u(v)");
    RNOK(       rcReadIf.getCode ( m_uiSliceGroupChangeCycle,
                                   getPPS().getLog2MaxSliceGroupChangeCycle( getSPS().getMbInFrame() ),
                                                                                   "SH: slice_group_change_cycle" ) );
	pcSyntax->setValue(m_uiSliceGroupChangeCycle);
  }

  const CommonControl* pcControlIf29 = pThialgouSlice->addControl("if", "!no_inter_layer_pred_flag && quality_id == 0", !getNoInterLayerPredFlag() && getQualityId() == 0);
  if( !getNoInterLayerPredFlag() && getQualityId() == 0 )
  {
	  pcSyntax = pcControlIf29->addSyntax("ref_layer_dq_id", "ue(v)");
    RNOK(       rcReadIf.getUvlc ( m_uiRefLayerDQId,                               "SH: ref_layer_dq_id" ) );
	pcSyntax->setValue(m_uiRefLayerDQId);

	const CommonControl* pcControlIf30 = pcControlIf29->addControl("if", "inter_layer_deblocking_filter_control_present_flag", getSPS().getInterlayerDeblockingPresent());
    if( getSPS().getInterlayerDeblockingPresent() )
    {
		pcMainH264->setEltCurrent(pcControlIf30);
      RNOK(     m_cInterLayerDeblockingFilterParameter.read( rcReadIf, getNalUnitType() == NAL_UNIT_CODED_SLICE_SCALABLE ) );
	  pcMainH264->setEltCurrent(pcEltCurrent);
    }

	pcSyntax = pcControlIf29->addSyntax("constrained_intra_resampling_flag", "u1(v)");
    RNOK(       rcReadIf.getFlag ( m_bConstrainedIntraResamplingFlag,              "SH: constrained_intra_resampling_flag"));
	pcSyntax->setValue(m_bConstrainedIntraResamplingFlag);

	const CommonControl* pcControlIf31 = pcControlIf29->addControl("if", "extended_spatial_scalability_idc == 2", getSPS().getExtendedSpatialScalability() == ESS_PICT);
    if( getSPS().getExtendedSpatialScalability() == ESS_PICT )
    {
		const CommonControl* pcControlIf32 = pcControlIf29->addControl("if", "ChromaArrayType > 0", 1);
		pcSyntax = pcControlIf32->addSyntax("ref_layer_chroma_phase_x_plus1_flag", "u(1)");
      RNOK(     rcReadIf.getFlag ( m_bRefLayerChromaPhaseXPlus1Flag,               "SH: ref_layer_chroma_phase_x_plus1_flag" ) );
	  pcSyntax->setValue(m_bRefLayerChromaPhaseXPlus1Flag);
	  pcSyntax = pcControlIf32->addSyntax("ref_layer_chroma_phase_x_plus1_flag", "u(2)");
      RNOK(     rcReadIf.getCode ( m_uiRefLayerChromaPhaseYPlus1, 2,               "SH: ref_layer_chroma_phase_y_plus1" ) );
	  pcSyntax->setValue(m_uiRefLayerChromaPhaseYPlus1);


	  pcSyntax = pcControlIf29->addSyntax("scaled_ref_layer_left_offset", "se(v)");
      RNOK(     rcReadIf.getSvlc ( m_iScaledRefLayerLeftOffset,                    "SH: scaled_ref_layer_left_offset" ) );
	   pcSyntax->setValue(m_iScaledRefLayerLeftOffset);
	    pcSyntax = pcControlIf29->addSyntax("scaled_ref_layer_top_offset", "se(v)");
      RNOK(     rcReadIf.getSvlc ( m_iScaledRefLayerTopOffset,                     "SH: scaled_ref_layer_top_offset" ) );
	  pcSyntax->setValue(m_iScaledRefLayerTopOffset);
	   pcSyntax = pcControlIf29->addSyntax("scaled_ref_layer_right_offset", "se(v)");
      RNOK(     rcReadIf.getSvlc ( m_iScaledRefLayerRightOffset,                   "SH: scaled_ref_layer_right_offset" ) );
	  pcSyntax->setValue(m_iScaledRefLayerRightOffset);
	   pcSyntax = pcControlIf29->addSyntax("scaled_ref_layer_bottom_offset", "se(v)");
      RNOK(     rcReadIf.getSvlc ( m_iScaledRefLayerBottomOffset,                  "SH: scaled_ref_layer_bottom_offset" ) );
	  pcSyntax->setValue(m_iScaledRefLayerBottomOffset);
    }
  }
  else
  {
  ...
  const CommonControl* pcControlIf33 = pThialgouSlice->addControl("if", "!no_inter_layer_pred_flag", !getNoInterLayerPredFlag());
  if( !getNoInterLayerPredFlag() )
  {
	   pcSyntax = pcControlIf33->addSyntax("slice_skip_flag", "u(1)");
    RNOK(       rcReadIf.getFlag ( m_bSliceSkipFlag,                               "SH: slice_skip_flag" ) );
	pcSyntax->setValue(m_bSliceSkipFlag);

	const CommonControl* pcControlIf34 = pcControlIf33->addControl("if", "slice_skip_flag", m_bSliceSkipFlag);
	const CommonControl* pcControlElseIf34 = pcControlIf33->addControl("else", "", !m_bSliceSkipFlag);
    if( m_bSliceSkipFlag )
    {
		pcSyntax = pcControlIf34->addSyntax("num_mbs_in_slice_minus1", "ue(v)");
      RNOK(     rcReadIf.getUvlc ( m_uiNumMbsInSliceMinus1,                        "SH: num_mbs_in_slice_minus1" ) );
	  pcSyntax->setValue(m_uiNumMbsInSliceMinus1);
    }
    else
    {
		pcSyntax = pcControlElseIf34->addSyntax("adaptive_base_mode_flag", "u(1)");
      RNOK(     rcReadIf.getFlag ( m_bAdaptiveBaseModeFlag,                        "SH: adaptive_base_mode_flag" ) );
	  pcSyntax->setValue(m_bAdaptiveBaseModeFlag);
	  const CommonControl* pcControlIf35 = pcControlElseIf34->addControl("if", "!adaptive_base_mode_flag", !m_bAdaptiveBaseModeFlag);
      if( !m_bAdaptiveBaseModeFlag )
      {
		  pcSyntax = pcControlIf35->addSyntax("default_base_mode_flag", "u(1)");
        RNOK(   rcReadIf.getFlag ( m_bDefaultBaseModeFlag,                         "SH: default_base_mode_flag" ) );
		pcSyntax->setValue(m_bDefaultBaseModeFlag);
      }

	  const CommonControl* pcControlIf36 = pcControlElseIf34->addControl("if", "!default_base_mode_flag", !m_bDefaultBaseModeFlag);
      if( !m_bDefaultBaseModeFlag )
      {
		   pcSyntax = pcControlIf36->addSyntax("adaptive_motion_prediction_flag", "u(1)");
        RNOK(   rcReadIf.getFlag ( m_bAdaptiveMotionPredictionFlag,                "SH: adaptive_motion_prediction_flag" ) );
		pcSyntax->setValue(m_bAdaptiveMotionPredictionFlag);
		const CommonControl* pcControlIf37 = pcControlIf36->addControl("if", "!adaptive_motion_prediction_flag", !m_bAdaptiveMotionPredictionFlag);
        if( !m_bAdaptiveMotionPredictionFlag )
        {
			pcSyntax = pcControlIf37->addSyntax("default_motion_prediction_flag", "u(1)");
          RNOK( rcReadIf.getFlag ( m_bDefaultMotionPredictionFlag,                 "SH: default_motion_prediction_flag" ) );
		  pcSyntax->setValue(m_bDefaultMotionPredictionFlag);
        }
      }

	  pcSyntax = pcControlElseIf34->addSyntax("adaptive_residual_prediction_flag", "u(1)");
      RNOK(     rcReadIf.getFlag ( m_bAdaptiveResidualPredictionFlag,              "SH: adaptive_residual_prediction_flag" ) );
	   pcSyntax->setValue(m_bAdaptiveResidualPredictionFlag);

	   const CommonControl* pcControlIf38 = pcControlElseIf34->addControl("if", " !adaptive_residual_prediction_flag", !m_bAdaptiveResidualPredictionFlag);
      if( !m_bAdaptiveResidualPredictionFlag )
      {
		   pcSyntax = pcControlIf38->addSyntax("default_residual_prediction_flag", "u(1)");
        RNOK(   rcReadIf.getFlag ( m_bDefaultResidualPredictionFlag,               "SH: default_residual_prediction_flag" ) );
		pcSyntax->setValue(m_bDefaultResidualPredictionFlag);
      }
    }

	const CommonControl* pcControlIf39 = pcControlElseIf34->addControl("if", "adaptive_tcoeff_level_prediction_flag", getSPS().getAVCAdaptiveRewriteFlag());
    if( getSPS().getAVCAdaptiveRewriteFlag() )
    {
		pcSyntax = pcControlIf39->addSyntax("tcoeff_level_prediction_flag", "u(1)");
      RNOK(     rcReadIf.getFlag ( m_bTCoeffLevelPredictionFlag,                   "SH: tcoeff_level_prediction_flag" ) );
	  pcSyntax->setValue(m_bTCoeffLevelPredictionFlag);
    }
  }
  else
  { 
  ...
  const CommonControl* pcControlIf40 = pThialgouSlice->addControl("if", "!slice_header_restriction_flag && !slice_skip_flag", !getSPS().getAVCHeaderRewriteFlag() && ! isH264AVCCompatible() && ! m_bSliceSkipFlag);
  if( !getSPS().getAVCHeaderRewriteFlag() && ! isH264AVCCompatible() && ! m_bSliceSkipFlag )
  {
	  pcSyntax = pcControlIf40->addSyntax("scan_idx_start", "u(4)");
    RNOK(       rcReadIf.getCode ( m_uiScanIdxStart, 4,                            "SH: scan_idx_start" ) );
	pcSyntax->setValue(m_uiScanIdxStart);
	pcSyntax = pcControlIf40->addSyntax("scan_idx_end", "u(4)");
    RNOK(       rcReadIf.getCode ( m_uiScanIdxStop,  4,                            "SH: scan_idx_end" ) );
	pcSyntax->setValue(m_uiScanIdxStop);
