                   ...
                   Bool           bReconstructAll )
{
  ...
  const CommonMb* pcThialgouMb = dynamic_cast<const CommonMb*>(pcMainH264->getEltCurrent());
  ASSERT(pcThialgouMb);
  ...
  // IDCT coeff. (Quantized)
  rcMbDataAccess.getMbTCoeffs().copyCoeffValueTo( cPredBuffer ); // method add by us
  pcThialgouMb->setData(CommonEltMbDataType_IDCTCoeffsQuant, 
			cPredBuffer.getMbLumAddr(), cPredBuffer.getLStride(), cPredBuffer.getLWidth(), cPredBuffer.getLHeight(),
			cPredBuffer.getMbCbAddr(), cPredBuffer.getCStride(), cPredBuffer.getCWidth(), cPredBuffer.getCHeight(),
			cPredBuffer.getMbCrAddr(), cPredBuffer.getCStride(), cPredBuffer.getCWidth(), cPredBuffer.getCHeight(),
			sizeof(XPel));

  // FIXME:
  if (pcMainH264->getCurrentLayerId() == 16 && pcMainH264->getCurrentPictureId() == (109-1) && rcMbDataAccess.getMbData().getMbAddr() == 352)
  {
	int	a = 0;
  }
  ...
  // IDCT coeff. (Scaled)
  rcMbDataAccess.getMbTCoeffs().copyCoeffValueTo( cPredBuffer ); // method add by us
  pcThialgouMb->setData(CommonEltMbDataType_IDCTCoeffsScale, 
			cPredBuffer.getMbLumAddr(), cPredBuffer.getLStride(), cPredBuffer.getLWidth(), cPredBuffer.getLHeight(),
			cPredBuffer.getMbCbAddr(), cPredBuffer.getCStride(), cPredBuffer.getCWidth(), cPredBuffer.getCHeight(),
			cPredBuffer.getMbCrAddr(), cPredBuffer.getCStride(), cPredBuffer.getCWidth(), cPredBuffer.getCHeight(),
			sizeof(XPel));
  ...
  if( rcMbDataAccess.getMbData().isIntra() )
  {
    ...
    else if( rcMbDataAccess.getMbData().getMbMode() == INTRA_BL )
    {
	  ...
	  // Thialgou: Final MB data loaded into xDecodeMbIntraBL()
    }
    else
    {
      ...
      rcMbDataAccess.getMbTCoeffs().copyPredictionFrom( cRecBuffer );
  	  ...
  	  pcThialgouMb->setData(CommonEltMbDataType_PreFilter, 
			cRecBuffer.getMbLumAddr(), cRecBuffer.getLStride(), cRecBuffer.getLWidth(), cRecBuffer.getLHeight(),
			cRecBuffer.getMbCbAddr(), cRecBuffer.getCStride(), cRecBuffer.getCWidth(), cRecBuffer.getCHeight(),
			cRecBuffer.getMbCrAddr(), cRecBuffer.getCStride(), cRecBuffer.getCWidth(), cRecBuffer.getCHeight(),
			sizeof(XPel));

	  // Thialgou: Compute residual (AVC)
	  cRecBuffer.subtract(cPredBuffer);
	  pcThialgouMb->setData(CommonEltMbDataType_Residual, 
				cRecBuffer.getMbLumAddr(), cRecBuffer.getLStride(), cRecBuffer.getLWidth(), cRecBuffer.getLHeight(),
				cRecBuffer.getMbCbAddr(), cRecBuffer.getCStride(), cRecBuffer.getCWidth(), cRecBuffer.getCHeight(),
				cRecBuffer.getMbCrAddr(), cRecBuffer.getCStride(), cRecBuffer.getCWidth(), cRecBuffer.getCHeight(),
				sizeof(XPel));
  ...
  // Predicted
  pcThialgouMb->setData(CommonEltMbDataType_Predicted, 
			cPredBuffer.getMbLumAddr(), cPredBuffer.getLStride(), cPredBuffer.getLWidth(), cPredBuffer.getLHeight(),
			cPredBuffer.getMbCbAddr(), cPredBuffer.getCStride(), cPredBuffer.getCWidth(), cPredBuffer.getCHeight(),
			cPredBuffer.getMbCrAddr(), cPredBuffer.getCStride(), cPredBuffer.getCWidth(), cPredBuffer.getCHeight(),
			sizeof(XPel));

  // Residual (SVC Base Layer)
  YuvMbBuffer  cResBuffer;
  if (pcBaseLayerResidual)
  {
	  cResBuffer.loadBuffer(pcBaseLayerResidual->getFullPelYuvBuffer() );
	  pcThialgouMb->setData(CommonEltMbDataType_ResidualBase, 
			cResBuffer.getMbLumAddr(), cResBuffer.getLStride(), cResBuffer.getLWidth(), cResBuffer.getLHeight(),
			cResBuffer.getMbCbAddr(), cResBuffer.getCStride(), cResBuffer.getCWidth(), cResBuffer.getCHeight(),
			cResBuffer.getMbCrAddr(), cResBuffer.getCStride(), cResBuffer.getCWidth(), cResBuffer.getCHeight(),
			sizeof(XPel));
  }
  if( !rcMbDataAccess.getMbData().isIntra() ) { // For I_BL, residual computed into "xDecodeMbIntraBL()", For AVC, residual computed above
	  cResBuffer.loadBuffer( pcResidualLF->getFullPelYuvBuffer() );
	  pcThialgouMb->setData(CommonEltMbDataType_Residual, 
				cResBuffer.getMbLumAddr(), cResBuffer.getLStride(), cResBuffer.getLWidth(), cResBuffer.getLHeight(),
				cResBuffer.getMbCbAddr(), cResBuffer.getCStride(), cResBuffer.getCWidth(), cResBuffer.getCHeight(),
				cResBuffer.getMbCrAddr(), cResBuffer.getCStride(), cResBuffer.getCWidth(), cResBuffer.getCHeight(),
                           ...
                           Frame*         pcBaseLayerRec )
{
  ...
  // FIXME: When "store_ref_base_pic_flag" is equal to zero then we don't need to reconstruct the base layer but we need it in Thialgou
  bReconstruct = true;
  ...
  if( pcRecYuvBuffer )
  {
	...
	pcThialgouMb->setData(CommonEltMbDataType_PreFilter, // not filtered yet
			cYuvMbBuffer.getMbLumAddr(), cYuvMbBuffer.getLStride(), cYuvMbBuffer.getLWidth(), cYuvMbBuffer.getLHeight(),
			cYuvMbBuffer.getMbCbAddr(), cYuvMbBuffer.getCStride(), cYuvMbBuffer.getCWidth(), cYuvMbBuffer.getCHeight(),
			cYuvMbBuffer.getMbCrAddr(), cYuvMbBuffer.getCStride(), cYuvMbBuffer.getCWidth(), cYuvMbBuffer.getCHeight(),
			sizeof(XPel));
  }
                             ...
                             YuvPicBuffer*  pcBaseYuvBuffer )
{
  ...
  // Thialgou: because "cYuvMbBuffer" is local
  pcThialgouMb->setData(CommonEltMbDataType_PreFilter, 
			cYuvMbBuffer.getMbLumAddr(), cYuvMbBuffer.getLStride(), cYuvMbBuffer.getLWidth(), cYuvMbBuffer.getLHeight(),
			cYuvMbBuffer.getMbCbAddr(), cYuvMbBuffer.getCStride(), cYuvMbBuffer.getCWidth(), cYuvMbBuffer.getCHeight(),
			cYuvMbBuffer.getMbCrAddr(), cYuvMbBuffer.getCStride(), cYuvMbBuffer.getCWidth(), cYuvMbBuffer.getCHeight(),
  ...
  // Thialgou: Compute residual
  cYuvMbBuffer.subtract(rcPredBuffer);
  pcThialgouMb->setData(CommonEltMbDataType_Residual, 
			cYuvMbBuffer.getMbLumAddr(), cYuvMbBuffer.getLStride(), cYuvMbBuffer.getLWidth(), cYuvMbBuffer.getLHeight(),
			cYuvMbBuffer.getMbCbAddr(), cYuvMbBuffer.getCStride(), cYuvMbBuffer.getCWidth(), cYuvMbBuffer.getCHeight(),
			cYuvMbBuffer.getMbCrAddr(), cYuvMbBuffer.getCStride(), cYuvMbBuffer.getCWidth(), cYuvMbBuffer.getCHeight(),
