                   ...
                   UInt&        ruiMbRead )
{
  ...
  for( ruiMbRead = 0; !bEndOfSlice; ruiMbRead++ ) //--ICU/ETRI FMO Implementation
  {
	...
	uiBitsLeft = m_pcMbParser->getBitsLeftCount(); // Thialgou
    RNOK( m_pcMbParser->read( *pcMbDataAccess, ruiMbRead, bEndOfSlice, uiNextSkippedVLC ) );
	pcMbDataAccess->getMbData().setBitsCount(uiBitsLeft - m_pcMbParser->getBitsLeftCount()); // Thialgou
