                                             ...
                                             Bool&                rbUseDefaultScalingMatrixFlag )
{
  ...
  Int   iDeltaScale;
  pcSyntax = pcEltCurrent->addSyntax("delta_scale", "");
  RNOK( pcReadIf->getSvlc( iDeltaScale,     "SCALING: delta_scale" ) );
  pcSyntax->setValue(iDeltaScale);
											...
											unsigned i)
{
  ...
  const CommonElt* pcEltCurrent = pcMainH264->getEltCurrent();
  ASSERT(pcEltCurrent != NULL);
  pcSyntax = pcEltCurrent->addSyntax("scaling_list_present_flag", "");
  RNOK  ( pcReadIf->getFlag( m_bScalingListPresentFlag,     "SCALING: scaling_list_present_flag" ) );
  pcSyntax->setValue(m_bScalingListPresentFlag);
  ROTRS ( ! m_bScalingListPresentFlag, Err::m_nOK );

  char ibuff[33] = {0};
  itoa(i, ibuff, 10);
  const CommonControl* pcControlIf0 = pcEltCurrent->addControl("if" , "seq_scaling_list_present_flag[ i/*="+std::string(ibuff)+"*/ ]", 1);
